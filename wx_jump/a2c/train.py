...
def train_net(game_net, eval_net, optimizer, eval_optimizer, device, batch_data):
    ...
    values = eval_net(states_v, torch.nn.functional.one_hot(actions_v.squeeze().long(), num_classes=ACTIONS))
    critic_loss_v = F.mse_loss(values.squeeze(-1), rewards_v)
    critic_loss_v.backward()
    eval_optimizer.step()

    logits_v = game_net(states_v)
...
def policy_agent(game_net, state):
    action = game_net(state)
    probs = F.softmax(action, dim=1).data.cpu().numpy()
    assert isinstance(probs, np.ndarray)
    pred_action = []
    for prob in probs:
        pred_action.append(np.random.choice(len(prob), p=prob))
    pred_action = np.array(pred_action)

    return pred_action, action.squeeze().data.cpu().numpy()

def prob_action_2_press_action(action):
    return 0.3 + 0.01 * action
...
def train(game_net, eval_net, game, writer, optimizer, eval_optimizer, scheduler, eval_scheduler, device, D = deque(), MINI_D = deque()):
    ...
    while True:
        ...
        while True:
            action, origin_action = policy_agent(game_net, torch.tensor(obs).unsqueeze(0).to(device))
            press_action = prob_action_2_press_action(action[0])

            new_s_t, r_t, done, new_obs = game.step_press_up(press_action)
        ...
        print(f"推理时间：{inference_dt:.2f}秒; 动作：{press_action}; 奖励：{r_t}")
        reward_count += r_t
        step_count += 1
        dt = time.time() - t
