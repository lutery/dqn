...
if __name__ == "__main__":
    ...
    env = TransposeObservation(gym.make(args.env, domain_randomize=True, continuous=False))
    test_env = TransposeObservation(gym.make(args.env, continuous=False))

    # 创建动作预测网络
    # 这里的获取游戏的动作空间使用env.action_space.n获取，是因为其是离散的动作空间
    # 而之前的游戏BipedalWalker是连续的动作空间，所以使用的是env.action_space.shape[0]
    # 所以两个获取动作空间的方式是不同的
    ...
    if (os.path.exists(os.path.join(save_path, "act-linear.pth"))):
        net_act.load_state_dict(torch.load(os.path.join(save_path, "act-linear.pth")))

    if (os.path.exists(os.path.join(save_path, "crt-linear.pth"))):
        net_crt.load_state_dict(torch.load(os.path.join(save_path, "crt-linear.pth")))
    ...
    with ptan.common.utils.RewardTracker(writer) as tracker:
        for step_idx, exp in enumerate(exp_source):
            ...
            torch.save(net_act.state_dict(), os.path.join(save_path, "act-linear.pth"))
            torch.save(net_crt.state_dict(), os.path.join(save_path, "crt-linear.pth"))
            trajectory.clear()
