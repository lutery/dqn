...
def distr_projection(next_distr_v, rewards_v, dones_mask_t, gamma, device="cpu"):
    ...
    for atom in range(N_ATOMS):
        # 这里就是将回报奖励压缩在-10到10之间，这个范围是根据经验而定，没有特殊的规定
        # 根据代码运行结果，一开始的回报只有-0.00n这个大小
        '''
        在基于分布的Q学习（如Categorical DQN）中，`Vmin` 和 `Vmax` 分别是预测的Q值分布的最小值和最大值。它们定义了我们预测Q值的范围，并且所有Q值原子都位于这个范围内。以下是如何选择这两个值的一些建议：

1. **基于经验估计**：您可以运行一个初步的训练过程或随机策略，并记录观察到的奖励。使用这些奖励，您可以估计可能的Q值的范围。然后，设置 `Vmin` 为观察到的最小Q值，设置 `Vmax` 为观察到的最大Q值。

2. **问题的特性**：如果您对环境有足够的了解，您可以基于奖励结构和折扣因子来估计Q值的范围。例如，如果奖励的最大可能值为 `R` 并且折扣因子为 `gamma`，那么在一系列的永续奖励情况下，Q值的上限大约是 `R / (1 - gamma)`。

3. **安全边界**：为了避免在初次估计中遗漏任何值，您可以为 `Vmin` 和 `Vmax` 设置宽松的边界。但是，这可能会导致Q值的分布在许多原子之间被稀释，从而减少学习的精确性。

4. **迭代调整**：随着训练的进行，您可以定期重新评估Q值的范围，并相应地调整 `Vmin` 和 `Vmax`。但是，请注意，如果您经常改变这些值，它可能会影响训练的稳定性。

5. **其他情况下的设置**：在某些环境中，奖励的范围可能是已知的。例如，在某些Atari游戏中，您可能知道每一步的奖励是固定的，例如 -1、0 或 1。在这种情况下，您可以根据最坏和最佳情况设定 `Vmin` 和 `Vmax`。

总之，为 `Vmin` 和 `Vmax` 选择合适的值是很重要的，因为这决定了Q值分布的分辨率和范围。最好根据具体问题的特性和实验结果来调整这些值。
        '''
...
if __name__ == "__main__":
    ...
    if (os.path.exists(os.path.join(save_path, "tgt-act-0.pth"))):
        tgt_act_net.target_model.load_state_dict(torch.load(os.path.join(save_path, "tgt-act-0.pth")))
        print("加载tgt_act模型成功")

    if (os.path.exists(os.path.join(save_path, "tgt-crt-0.pth"))):
        tgt_crt_net.target_model.load_state_dict(torch.load(os.path.join(save_path, "tgt-crt-0.pth")))
        print("加载tgt_crt模型成功")
        ...
        with ptan.common.utils.TBMeanTracker(writer, batch_size=10) as tb_tracker:
            while True:
                ...
                if frame_idx % TEST_ITERS == 0:
                    ...
                    # 保存模型
                    torch.save(act_net.state_dict(), os.path.join(save_path, "act-0.pth"))
                    torch.save(crt_net.state_dict(), os.path.join(save_path, "crt-0.pth"))
                    torch.save(tgt_act_net.target_model.state_dict(), os.path.join(save_path, "tgt-act-0.pth"))
                    torch.save(tgt_crt_net.target_model.state_dict(), os.path.join(save_path, "tgt-crt-0.pth"))
