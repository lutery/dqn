#!/usr/bin/env python3
'''
验证通过，可以正常的执行训练，并且其损失可以i正常的收敛，奖励会逐渐增加，和书中的一样，要训练比较长的时间步数才行，我这边的测试结果要在230K时，loss_actor才会下降到负区域，然后一直下降，虽然奖励也会出现下降趋势，但是放眼更大的区域，奖励总体是上升的趋势
'''
...
class TD3():
    def __init__(self, obs_space, device="cpu"):
        self.device = device
        ...
        self.agent = model.AgentDDPG(self.act_net, device=device)
...
if __name__ == "__main__":
    ...
    # 构建动作网络和评价网络
    td3 = TD3(env.observation_space, device=device)
    print(td3.act_net)
    print(td3.crt1_net)
    print(td3.crt2_net)

    writer = SummaryWriter(comment="-ddpg_" + args.name)
    exp_source = ptan.experience.ExperienceSourceFirstLast(env, td3.agent, gamma=GAMMA, steps_count=1)
